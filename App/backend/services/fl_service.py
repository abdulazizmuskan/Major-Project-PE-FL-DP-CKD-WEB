...
class FederatedLearningService:
        ...
        try:
            ...
            try:
                ...
                if all_X_val and all_y_val:
                    ...
                    # Validate improvement: rollback if worse
                    if previous_accuracy is not None and global_accuracy is not None:
                        if global_accuracy < previous_accuracy:
                            print(f"[AGGREGATION] WARNING: Accuracy dropped ({global_accuracy:.4f} < {previous_accuracy:.4f}), rolling back...")
                            global_model.load_state_dict(original_state)
                            global_accuracy = previous_accuracy
                            print(f"[AGGREGATION] Rolled back. Keeping previous accuracy: {previous_accuracy:.4f}")
                        else:
                            print(f"[AGGREGATION] Model improved: {previous_accuracy:.4f} → {global_accuracy:.4f}")
                    
                    self.save_model(global_model, str(global_model_path))
                else:
                    print(f"[AGGREGATION] Warning: No validation data available, saving model without evaluation")
                    self.save_model(global_model, str(global_model_path))
            except Exception as eval_error:
                print(f"[AGGREGATION] ERROR during evaluation: {str(eval_error)}")
                import traceback
                traceback.print_exc()
                self.save_model(global_model, str(global_model_path))
            ...
            # Copy global model to all clients as predict_model and reset status
            for client in clients:
                client_id = client['id']
                client_dir = self.get_client_dir(client_id)
    ...
    def predict(self, client_id: int, patient_data: Dict, template_csv: str) -> Dict:
        ...
        try:
            # ── FIX 1: Clean up missing / empty values sent from the frontend ──────
            # The form sends '-' or '' for fields left blank. Convert those to NaN
            # so pandas/imputation handles them correctly downstream.
            cleaned_data = {}
            for k, v in patient_data.items():
                if v in ('-', '', None, 'nan', 'NaN', 'null'):
                    cleaned_data[k] = float('nan')
                else:
                    try:
            ...
            # Load predict model for this client (fall back to global if missing)
            client_dir = self.get_client_dir(client_id)
            predict_model_path = client_dir / "predict_model.pt"
            ...
            # Load template CSV to get feature columns
            resolved_template_csv = self.resolve_dataset_path(template_csv)
            ...
            # Impute missing values with template column mean
            for c in feature_cols:
                if pd.isna(patient_df[c].iloc[0]):
                    patient_df.loc[0, c] = template_df[c].mean()
            
            # Z-score capping (clip extreme outliers using template statistics)
            z_threshold = 4.0
            for c in feature_cols:
                try:
            ...
            with torch.no_grad():
                ...
                # --- THE CRITICAL FIX ---
                # In your dataset (likely the UCI CKD dataset):
                # Class 0 = CKD (Chronic Kidney Disease)
                # Class 1 = NotCKD (Healthy)
                #
                # Therefore, if prob > 0.5, the model is sure it is HEALTHY.
                # If prob < 0.5, the model is sure it is CKD.
                
                # --- EMERGENCY FLIP ---
                # Use this if your model was trained with CKD=1 and Healthy=0
                if prob > 0.5:
                    prediction = 'ckd'        # HIGH probability now means Sick
                    confidence = prob
                else:
                    prediction = 'not_ckd'    # LOW probability now means Healthy
                    confidence = 1.0 - prob
            ...
            """, (
                ...
                prediction,
                float(confidence)   # ── FIX 5: was float(prob) — now correctly float(confidence)
            ))
            conn.commit()
            conn.close()
            ...
            return {
                ...
                "confidence": float(confidence),   # ── FIX 5 (same): was float(prob)
